  ...
  return (
    <div className="min-h-screen pb-24">
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            {currentStorageId && (
              <button
                onClick={() => navigate('/')}
                className="bg-pink-100 hover:bg-pink-200 text-pink-600 p-2 rounded-xl transition-colors"
              >
                <BackIcon className="w-5 h-5" />
              </button>
            )}
            <h1 className="text-2xl font-bold text-gray-800 tracking-tight">
              {currentStorageId && activeStorage
                ? activeStorage.name
                : currentStorageId === 'all'
                  ? 'All Items'
                  : 'ClosetCutie 🎀'}
            </h1>
          </div>
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 bg-pink-50 border-none rounded-full text-sm focus:ring-2 focus:ring-pink-300 w-32 sm:w-48 transition-all"
            />
          </div>
        </div>
      </header>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 bg-gray-800 text-white p-4 rounded-full shadow-lg shadow-purple-300 hover:scale-110 active:scale-95 transition-all z-40 border-4 border-white"
      >
      ...
      <AddItemModal
        ...
        preselectedStorageId={
          currentStorageId && currentStorageId !== 'all' ? currentStorageId : undefined
        }
      />
          ...
          <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden relative shadow-2xl animate-pop-in">
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="bg-gray-100 rounded-lg px-2 py-1 text-xs font-bold text-gray-500">
                  {selectedItem.color}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedItem.tags.map((tag) => (
                  <span
                    key={tag}
                    className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm font-bold"
                  >
                    #{tag}
                  </span>
                ))}
